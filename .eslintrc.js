module.exports = {
    ...
    env: {                  // 此项指定环境的全局变量，下面的配置指定为浏览器环境
        node: true,
        /* "browser": true, "node": true, "commonjs": true, "es6": true, "amd": true */
    },
    parser: 'vue-eslint-parser',             //此项是用来指定eslint解析器的，解析器必须符合规则，babel-eslint解析器是对babel解析器的包装使其与ESLint解析
    ...
    extends: 'plugin:vue/essential',                          // 此项是用来配置标准的js风格，就是说写代码的时候要规范的写，如果你使用vs-code我觉得应该可以避免出错

    /*  plugins: [ 'html', "flow-vars", "react" ], */         // 此项是用来提供插件的，插件名称省略了eslint-plugin-，下面这个配置是用来规范html的
    /* 
