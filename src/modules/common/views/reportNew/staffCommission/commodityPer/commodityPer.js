...
export default {
        ...
        return {
            ...
            gridOption: {
                ...
                columnData: [
                    ...
                    { key: 'refundMoney', text: '退款提成', align: 'center' },
                    { key: 'showCalcMoney', text: '提成', align: 'center' },
    ...
    computed: {
        ...
        queryObj() {
            let { CommissionType, EmployeeId, IsRefund, PaymentFilter, ProductFilter, ShopIds, PtIdS, StartDate, EndDate, PageIndex, PageSize } = this.queryEntity;
            let obj = { CommissionType, EmployeeId, IsRefund, PaymentFilter, ProductFilter, ShopIds, PtIdS, StartDate, EndDate, PageIndex, PageSize };
    ...
    mounted() {
        if (this.userInfo.sv_us_industrytype !== 1) {
            setTimeout(() => {
                this.$gridManager.hideTh('commodityPer', 'isAssign');
            }, 100)
        }
    ...
    methods: {
        ...
        ProductCommission() {                               // 获取商品业绩数据
            stockApi.ProductCommission(this.queryObj).then(res => {
                this.$gridManager.setAjaxData('commodityPer', {
