...
export default {
    ...
    data() {
        return {
            ...
            total: 0,
            gridOption: {
                gridManagerName: 'sellingCardsAch',
                ...
                columnData: [
                    ...
                    { key: 'productName', text: '卡种', align: 'center' },
                    ...
                    { key: 'amount', text: '退款业绩', align: 'center' },
                    { key: 'calcMoney', text: '业绩', align: 'center' },
    ...
    computed: {
        ...
        queryObj() {
            let { CommissionType, EmployeeId, IsRefund, ShopIds, PtIdS, PaymentFilter, StartDate, EndDate, PageIndex, PageSize } = this.queryEntity;
            let obj = { CommissionType, EmployeeId, IsRefund, ShopIds, PtIdS, PaymentFilter, StartDate, EndDate, PageIndex, PageSize };
    ...
    methods: {
        ...
        handleDownload() {                                  // 导出
            stockApi.SetmealRechargeCommissionDownload(this.queryObj).then(res => {
        ...
        SetmealRechargeCommission() {                       // 获取商品业绩数据
            stockApi.SetmealRechargeCommission(this.queryObj).then(res => {
                this.$gridManager.setAjaxData('sellingCardsAch', {
                    data: res.datas.map(item => {
                        let calcMoney = item.isRefund ? 0 : item.calcMoney;
                        let amount = item.isRefund ? item.refundMoney : 0;
                        let createTime = this.$app.isNull(item.createTime) ? '' : this.$app.currentTime(new Date(item.createTime), 'yyyy-MM-dd HH:mm:ss');
                        return { ...item, calcMoney, amount, createTime }
                    })
                });
