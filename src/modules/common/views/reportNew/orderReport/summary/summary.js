...
export default {
    ...
    computed: {
        ...
        courseValue() {
            ...
            const one = courseStructInfo.find(item => item.name == '私教课') || {}
            const more = courseStructInfo.find(item => item.name == '团体课') || {}
            ...
            return stockApi.GetOrderProduct(this.queryEntity).then(res => {
                ...
                if (one_for_one > 0 || one_to_many > 0) {
                    this.courseStructInfo = [
                        { value: +one_for_one, name: "私教课", itemStyle: { color: '#506FEE' } },
                        { value: +one_to_many, name: "团体课", itemStyle: { color: '#e75235' } },
