...
export default {
    components: { numberChange, popTableList, popTableAnd, carttingCoffee, carttingCoffeeMore, carttingPreOrder, goodsList, memberAdd, memberRecharge, cateringReturn, categoryFirstAdd, categorySecondAdd, updateBillabletimeSelect },
    name: 'c-cashier',
    props: {
        dataJson: {
            type: Object,
    ...
    data() {
        return {
            ...
            tableInfo: {
                ...
                sv_table_id: null,
                booking_type: 0,                                // 0-普通 1-预付款 : 2-预约时间
                sv_region_name: '',
                sv_employee_id: 0,
    ...
    computed: {
        ...mapState(['isCefClient', 'userInfo', 'caleStep', 'carttingData', 'takeASingleCale3', 'cashierJurisdiction', 'memberInfo', 'couponCountNumber', 'selectedInfo', 'pricingScaleData', 'user_local_data', 'carttingSearchCursor', 'goodsEmployeeList', 'customerDisplayData']),
        ...
        post_guadan() {                                             // 开台后下单
            ...
            let prlist = this.carttingData.productResults.map((e, i) => {
                return {
                    ...
                    without_project_status: this.goodsEmployeeList[i].employeeList.length > 0 ? this.goodsEmployeeList[i].employeeList[0].status : 0,
                    without_project_start_time: this.goodsEmployeeList[i].employeeList.length > 0 ? this.goodsEmployeeList[i].employeeList[0].start_time : '',
                    without_project_end_time: this.goodsEmployeeList[i].employeeList.length > 0 ? this.goodsEmployeeList[i].employeeList[0].end_time : '',
                    multiple_provider_list: this.goodsEmployeeList[i].employeeList.length === 0 ? null : this.goodsEmployeeList[i].employeeList.map(workerItem => {
