<template>
    <div class="cashier privateRoom" ref="restaurant" tabindex="0" @keyup.enter.stop="handleEnter" @click="handleCashier">
                ...
                <div class="billabletimeWrap" v-if="!$app.isNull(billabletimeInfo.id)">
                    ...
                    <div class="wrapStatus">
                        ...
                        <template v-else>
                            ...
                            <div class="preknot_time" v-if="tableInfo.preknot_time">
                                <span>自动预结时间：</span>
                                <span class="timeText" @click="handleShowUpdatePreknotTime">
                                    {{ tableInfo.preknot_time.substring(11) }}
        ...
        <!-- 弹窗修改自动预结时间 -->
        <dc-dialog v-if="popPreknotTime" width="600" height="300" title="自动预结账延时" @close="popPreknotTime = false">
            <div class="popPreknotTime">
                <div class="tips_content">
                    <div class="contentLine">
                        <span class="key">原自动预结时间：</span>
                        <span>{{ tableInfo.preknot_time }}</span>
                    </div>
                    <div class="contentLine">
                        <span class="key">增加时长：</span>
                        <el-select class="selectBox" v-model="sv_preknot_hour" @change="handleUpdatePreknotSelect">
                            <el-option :label="0" :value="0"></el-option>
                            <el-option v-for="(item, index) in 23" :label="item" :value="item" :key="index"></el-option>
                        </el-select>
                        <span class="text hour">小时</span>
                        <el-select class="selectBox" v-model="sv_preknot_minute" @change="handleUpdatePreknotSelect">
                            <el-option :label="0" :value="0"></el-option>
                    ...
                    <div class="contentLine">
                        <span class="key">修改后预结时间：</span>
                        <span>{{ update_preknot_time }}</span>
                    </div>
                </div>
                <div class="tips_btnWrap">
                    <div class="btn" v-repeatClick @click="handleUpdatePreknotTime">确定</div>
                </div>
            </div>
        </dc-dialog>
        <!-- 结束提示 -->
