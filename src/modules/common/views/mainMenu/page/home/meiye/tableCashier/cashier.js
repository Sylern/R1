...
import popTableList from '@/modules/common/views/common/base/popTableList/popTableList.vue';
import popTableAnd from '@/modules/common/views/common/base/popTableAnd/popTableAnd.vue';
import carttingMeiyeTable from '@/modules/common/views/common/base/carttingMeiyeTable/carttingMeiyeTable.vue';
import carttingCoffeeMore from '@/modules/common/views/common/base/carttingCoffeeMore/carttingCoffeeMore.vue';
...
import updateBillabletimeSelect from '@/modules/common/views/common/base/updateBillabletimeSelect/updateBillabletimeSelect.vue';
export default {
    components: { numberChange, popTableList, popTableAnd, carttingMeiyeTable, carttingCoffeeMore, cashierRight, memberAdd, memberRecharge, cateringReturn, updateBillabletimeSelect },
    name: 't-cashier',
    props: {
        dataJson: {
            type: Object,
    ...
    mounted() {
        ...
        this.$root.$on('tableUpdateMember', (item) => {
            if (this.orderData.productResults.length > 0) {
                this.tableUpdateMember = item.member_id;
                this.takeASingleUpdateCatering(this.orderData.productResults);
            }
        });
    ...
    beforeDestroy() {
        ...
        this.$root.$off('tableUpdateMember');
    ...
    methods: {
        ...
        pageFocus() {
            this.$nextTick(() => {
                this.$refs.tableCashier && this.$refs.tableCashier.focus();
            })
        },
        handleCloseBillable() {
        ...
        handleBack() {                                              // 返回房台
            ...
            if (this.carttingData.productResults.length !== 0 && this.tableInfo.sv_table_using_state !== 4) {
                this.$confirm('您有未下单的商品，返回后将清空，是否返回？', '提示', {
        ...
        startBillableTimer() {
            if (this.billabletimeInfo.status === 1 || this.billabletimeInfo.status === 2) {
                if (this.billabletimeInfo.configId !== this.tableInfo.sv_billable_id) return
                console.log('start');
                clearTimeout(this.billableTimer);
                this.billableTimer = null;
                const cycleTime = 60000;
                this.billableTimer = setTimeout(() => { this.getBillabletime(this.startBillableTimer) }, cycleTime)
            }
        },
        ...
        handleReturnNumberChange(data) {                            // 退菜数量填写返回
            ...
            let item = dataList[this.$refs.carttingMeiyeTable.getOrderListPos()];
        ...
        handleUpdateTable(val) {                                    // 0 换台 1 并台 2 移菜
            ...
            if (val.type === 2) {
                ...
                let query = {
                    ...
                    list_id: [this.orderData.productResults[this.$refs.carttingMeiyeTable.currentPos].sv_without_product_id],
        ...
        caterCallback(type) {                                       // 单品起菜
            ...
            if (type === 2) {
                // 起菜
                if (this.orderData.productResults[this.$refs.carttingMeiyeTable.getOrderListPos()].sv_is_rouse !== 1) return this.$message.warning('菜品不需要起菜')
                stockApi.updateTableRouse({
                    tableId: this.tableInfo.sv_table_id,
                    sv_is_rouse: 2,
                    sv_without_product_ids: [this.orderData.productResults[this.$refs.carttingMeiyeTable.getOrderListPos()].sv_without_product_id]
                }).then(res => {
                    if (res) {
                        this.$message.success('起菜成功');
                        this.orderData.productResults[this.$refs.carttingMeiyeTable.getOrderListPos()].sv_is_rouse = 2;
                        this.getKitchenPrinter([this.orderData.productResults[this.$refs.carttingMeiyeTable.getOrderListPos()]], 'unWait');
                    }
                });
                return
            }
        ...
        handleBtnControl(type) {                                    // 订单按钮回调
            ...
            if (type === 2) {
                // 催菜
                let dataList = [];
                if (this.$refs.carttingMeiyeTable.getOrderListPos() === -1) {
                    dataList = this.orderData.productResults;
                } else {
                    dataList.push(this.orderData.productResults[this.$refs.carttingMeiyeTable.getOrderListPos()]);
                }
            ...
            if (type === 3) {
                // 退菜
                if (this.$refs.carttingMeiyeTable.getOrderListPos() === -1) return this.$message.warning('请选择要退的菜品')
                if (this.orderData.productResults[this.$refs.carttingMeiyeTable.getOrderListPos()].sv_return_status === 2) return this.$message.warning('选择菜品已退')
                let dataList = JSON.parse(JSON.stringify(this.orderData.productResults));
                let item = dataList[this.$refs.carttingMeiyeTable.getOrderListPos()];
        ...
        showMemberAdd(dataObj) {                                    // 显示新增会员弹窗
            ...
            if (this.$app.isNull(dataObj.val)) {
                this.memberAddImport = null;
            }
        },
        showStoreCard() {
        ...
        getBillabletime(fn) {
            stockApi.getBillabletime({ code: this.tableInfo.billabletime_code, dateTime: this.tableInfo.sv_table_prekont_date }).then(res => {
                if (res) {
        ...
        getCateringModel() {                                        // 获取房台基础信息
            ...
            stockApi.getCateringModel(query).then(res => {
                if (res) {
                    ...
                    if (!this.$app.isNull(this.tableInfo.billabletime_code)) this.getBillabletime();
                }
            });
        },
        ...
        takeASingleUpdateCatering(dataList, tips, callBackFn) {     // 修改挂单 
            ...
            let query = {
                ...
                sv_table_id: this.tableInfo.sv_table_id,
                user_id: this.userInfo.user_id,
                member_id: !this.$app.isNull(this.tableUpdateMember) ? this.tableUpdateMember : this.memberInfo.member_id,
                sv_table_is_together: false,                        // 是否拼桌
                sv_order_data_type: 0
            }
        ...
        addToOrder() {                                              // 加菜
            ...
            let prlist = dataList.map((e, i) => {
                return {
                    ...
                    sv_return_status: e.sv_return_status || 0,
                    product_num: e.number,
                    sv_preferential: e.sv_preferential || null,
                    product_unitprice: this.$app.isNull(e.productChangePrice) ? e.price : e.productChangePrice,
                    sv_product_is_change: this.$app.isNull(e.productChangePrice) ? false : true,
                    sv_product_is_package: this.$app.isNull(e.packageGroups) ? false : true,
                    combination_new: this.$app.isNull(e.packageGroups) ? '' : JSON.stringify(e.packageGroups),
        ...
        handelFrontPrint(dataList, printType) {                     // 前台 宾客单
            // printType 1宾客单  
            ...
            if (this.userInfo.sv_us_industrytype !== 27) {
                printTypeText = ['', '宾客单', '加单', '退单', '换台单', '并台单'];
            }
            let printDataList = [];
