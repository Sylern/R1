...
import tCashier from '../tableCashier/tableCashier.vue';
...
export default {
	components: { numberChange, tCashier, reservation, reservationList, reservationInfo },
	name: 'deskList',
	data() {
		return {
	...
	computed: {
		...mapState(['refreshRoom', 'userInfo', 'isCefClient']),
	...
	methods: {
		...mapMutations(['update', 'clearCartting', 'clearBillabletimeInfo']),
		...
		//#region 获取数据
		getbizemployeeList() {                               // 获取服务中服务人员
			stockApi.getbizemployeeList().then(res => {
				if (res) {
		...
		handleLightSwitch(item) {
			...
			stockApi.updateElectricSwitch({ sv_electric_id: item.sv_electric_id, sv_table_id: item.sv_table_id, sv_switch: currentSwitch ? false : true }).then(res => {
				if (res) {
					let filterItem = this.dataList.find(e => e.sv_electric_id === item.sv_electric_id)
