...
<template>
    <div class="cashierRight">
        <div class="typeWrap">
            <div class="btnItem" :class="{ 'selected': item.type === propsProducttype }" v-for="(item, index) in topBtnType" :key="index" @click="handleTopBtn(item)">
                <cmd-icon :type="item.icon" size="28" color="#3333333"></cmd-icon>
                <span class="typeBtnName">{{ item.name }}</span>
            </div>
        </div>
        <div class="classification" v-if="propsProducttype !== '5'">
            <div class="c_one">
        ...
        <div class="goodsWrap">
            <goodsList ref="goodsList" darkModel :producttype="propsProducttype" :categoryId="firstSelectedItem.id" :erjicategory="secondSelectedItem.id" />
        </div>
        <!-- 新增一级分类 -->
