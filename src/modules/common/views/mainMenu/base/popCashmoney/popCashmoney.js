export default {
    ...
    methods: {
        ...
        handleFocus({ target }) {
            target.value = this.$app.isNull(target.value) ? '' : parseFloat(target.value);
        },
        handleInput({ target }) {
        ...
        handleSure() {                                              // 确定按钮点击事件
            if (this.inputNumber > 999999) return this.$message.warning('输入不能超过999999');
            if (this.$app.isNull(this.inputNumber) || this.inputNumber < 0) return this.$message.warning('请输入备用金，备用金不能小于0');
