...
export default {
    ...
    watch: {
        dialogVisible: {
            handler(newVal, oldVal) {
                if (newVal) {
                    ...
                    this.$nextTick(() => {
                        !!this.$refs.checkCourse && this.$refs.checkCourse.focus();
                    });
                } else {
                    !!this.$refs.checkCourse && this.$refs.checkCourse.blur();
                }
            }
        },
        memberSelected: {
        ...
        memberListStatus: {
            handler(newVal, oldVal) {
                if (newVal) {
                    this.$refs.checkCourse.blur();
                } else {
                    this.$refs.checkCourse.focus();
                }
            }
        },
        ...
        'memberInfo.member_id': {
            handler(newVal, oldVal) {
                this.checkMoney = this.$app.addNumber(this.importInfo.courseMoney, this.carttingData.dealMoney);
                this.inputNumber = this.$app.addNumber(this.importInfo.courseMoney, this.carttingData.dealMoney);
            }
        },
    },
    methods: {
        ...
        getMemberOrderCalss(callback) {                             // 查询学员待选班课程
            if (this.$app.isNull(this.classSelectionOrderId)) return
            let query = {
                o_id: this.classSelectionOrderId
            }
            stockApi.getMemberOrderCalss(query).then(res => {
                ...
                if (res) {
                    ...
                    this.classSelectionStatus = true;
                }
            });
        },
        submitSuccess(isBack) {                                     // 结算成功回调
            if (isBack) return this.successHandle();
            this.getMemberOrderCalss(() => {
                this.successHandle();
            });
        },
        successHandle() {
        ...
        doSubmit() {                                                // 执行结算
            ...
            let list = this.dataList.map((e, i) => {
                return {
                    ...
                    sv_course_packageid: e.sv_course_packageid,
            ...
            stockApi.mixedSettleOrder(query, this.Forbid).then(res => {
                this.isSubmitting = false;
