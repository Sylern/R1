<!--棋牌并台弹窗-->
<template>
    <div class="popTableAnd" ref="popTableAnd" v-if="dialogVisible" @keyup.stop="" @keyup.esc.stop="closeDialog">
        <dc-dialog width="1200" height="650" :title="dataObj.title" @close="closeDialog">
            ...
            <div class="contentWrap">
                <div class="scrollListWrap">
                    <el-scrollbar ref="scrollbar" class="srcollList">
                        <div class="listWrap" v-if="!$app.isNull(dataList)">
                            <div class="listItem" :class="{'selected': checkList.map(e => e.sv_table_id).includes(item.sv_table_id)}" v-for="(item, index) in searchResult" :key="index" @click="handleListTable(item)">
                ...
                <div class="btnWrap">
                    <el-button class="btnItem" :disabled="checkList.length === 0" type="primary" v-repeatClick @click="handleBtnSure">确定</el-button>
                </div>
            </div>
        </dc-dialog>
    </div>
...
@import './popTableAnd.scss';
</style>
<script src="./popTableAnd.js"></script>