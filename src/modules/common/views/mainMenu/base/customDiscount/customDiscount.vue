    ...
    <div class="customDiscount" ref="customDiscount" v-if="dialogVisible" tabindex="0" @keyup.stop="listenKeyup">
        <dc-dialog width="4.68" height="8.1" title="自定义" @close="closeDialog">
            <div class="contentWrap">
                <div v-for="(item,index) in currentDiscountList" :key="index">
                    <el-input class="inputWrap" :class="{'selected': inputWrapPos === index}" :ref="'inputMoney'+index" v-model.trim="item.value" @focus="handleFocus($event,index)" @keyup.native.stop="handlePrevent" @blur="handleInputBlur" @input.native.prevent="handleInput" @clear="clearInput(index)" clearable>
