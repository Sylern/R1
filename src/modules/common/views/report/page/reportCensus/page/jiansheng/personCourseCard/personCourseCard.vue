            ...
            <div class="rep_q_bottom">
                <div class="rep_qb_from">
                    <div class="rep_qbf_item">
                        <div class="rep_qbfi_name">私教模式：</div>
                        <div class="rep_qbfi_label">
                            <el-select v-model="queryEntity.Course_type" placeholder="选择私家模式" @change="handleSearch">
                    ...
                    <div class="rep_qbf_item">
                        <div class="rep_qbfi_name">到期时间：</div>
                        <div class="rep_qbfi_label">
                            <date-time-picker allSelect :paramData="[queryEntity.sv_start_expiration_datetime, queryEntity.sv_end_expiration_datetime]" @change="handleChangeTime($event, 'expiration')"></date-time-picker>
                        </div>
                    </div>
                </div>
                <div class="rep_qb_from">
                    <div class="rep_qbf_item">
                        ...
                        <div class="rep_qbfi_label">
                            <date-time-picker allSelect :paramData="[queryEntity.sv_start_datetime, queryEntity.sv_end_datetime]" @change="handleChangeTime($event)"></date-time-picker>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="req_table">
            <div class="tableBox SheetTable">
                <el-table ref="multipleTable" :data="dataList" tooltip-effect="dark" height="100%" style="width: 100%;" @selection-change="handleSelectionChange">
                    ...
                    <el-table-column prop="sv_mr_name" label="会员" width="110" align="left"> </el-table-column>
                    <el-table-column prop="sv_mr_mobile" label="手机号码" width="120" align="left"> </el-table-column>
                    <el-table-column prop="sv_p_name" label="课程名" width="120">
                        <template v-slot:default="{ row }">
                            <div>
                                <el-tag color="rgba(var(--main-theme-color), 1);" size="mini" effect="dark">{{ row.sv_course_type_name }}</el-tag>
                            </div>
                            <div>{{ row.sv_p_name }}</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="sv_order_datetime" label="购买时间" width="160" align="left">
                        <template v-slot:default="{ row }">
                            <div> <span :class="row.sv_open_card_state ? 'status' : 'status-err'"></span> {{ row.sv_open_card_state?'已开卡':'未开卡' }} </div>
                    ...
                    <el-table-column prop="sv_price" label="单价" width="80" align="right">
                        <template v-slot:default="{ row }">
                            ￥{{ $app.moneyFixed(row.sv_price,2) }}
                        </template>
                    </el-table-column>
                    <el-table-column prop="sv_coach" label="上课教练" width="120" align="left"></el-table-column>
                    <el-table-column prop="salesperson" label="销售" width="110" align="left"></el-table-column>
            ...
            <div class="pageWrap" v-if="total > 0 ">
                <div class="summary">
                    ...
                    <div class="num">
                        总余额：<span>{{ summary.sv_Total_balance }}</span>
                        购课：<span>{{ summary.sv_Total_purchase }}</span>
                        剩余摊销（元）：<span>{{ $app.moneyFixed(summary.sv_RemainingAmortization,2) }}</span>
                        实收（元）：<span>{{ $app.moneyFixed(summary.sv_ActualAmount,2) }}</span>
                    </div>
                </div>
...
<script src="./personCourseCard"></script>
<style lang="scss" scoped>
@import './personCourseCard.scss';
</style>
