...
export default {
        ...
        return {
            ...
            rulesLoginForm: {
                ...
                user_psw: [{
                    ...
                    message: '请输入您的密码',
                    trigger: ['change']
                }, { min: 6, max: 15, message: '请设置登录密码（8-32位数字+字母+符号组合）', trigger: 'blur' }],
                picture_code: [{
                    required: true,
                    message: '请输入图片验证码',
                    trigger: ['change']
    ...
    mounted() {
        this.verifi_code_time = parseInt(window.localStorage.getItem('register_getVerifi_code_time')) || this.verifi_code_time;
        // 验证码倒计时
    ...
    beforeDestroy() {
        window.localStorage.setItem('register_getVerifi_code_time', this.verifi_code_time);
    },
    methods: {
        ...
        onPageService() {                           // 《服务条款》
            window.location.href = this.$app.getOrigin() +'/vueview/private/'+ (this.loginPageType === 0 ? 'agreement3.html' : 'agreement4.html');
        },
        onPagePrivacy() {                           // 《私隐条款》
            window.location.href = this.$app.getOrigin() +'/vueview/private/'+ (this.loginPageType === 0 ? 'private3.html' : 'private4.html');
        },
        handleLogin() {                             // 跳转到登录页面
            this.$router.push('/login');
        },
        handleSelected() {                          // 选择行业返回
            ...
            this.$refs.loginForm.validate((valid) => {
                if (valid) {
                    if (this.agreementStatus) {
                        const sv_ul_loginpwd = this.$app.md5(this.loginForm.user_psw);
                        const sv_sharechannel = this.$app.getQueryStringFirst('_key');
                        let regsource = 'webclientapi';
                        if (window.location.href.indexOf('bd_vid') !== -1) {
                            const val = this.$app.getQueryStringSecond('bd_vid');
                            regsource += '|bd_vid=' + val;
                        }
                        basicApi.postWebUserRegister(this.loginForm.verifi_code, { sv_ul_mobile: this.loginForm.user_id, sv_ul_loginpwd, sv_regsource: regsource, sv_sharechannel, distributor_code: this.userTypeCode, usertypecode: this.userTypeCode }).then(res => {
                            if (res) {
        ...
        getWebGetSiteConfig() {                     // 获取站点配置  判断德客或oem
            basicApi.GetDstributorConfig({ url: this.$app.getHost() }).then(res => {          // 获取站点配置  判断德客或oem
                ...
                if (res) {
                    ...
                    this.bgUrl = res.sv_background_image ? res.sv_background_image.replace('http://', '//') : 'https://ds.decerp.cc/images/base/login/background.png';
